...
export function TrainersCrud({ trainers }: { trainers: Trainer[] }) {
  return (
    <ResourceCrud
      ...
      columns={[
        { label: "Trainer", render: (trainer) => trainer.full_name },
        { label: "Specialty", render: (trainer) => trainer.specialty ?? "-" },
        { label: "Phone", render: (trainer) => trainer.phone ?? "-" },
        { label: "Status", render: (trainer) => (trainer.is_active ? "Active" : "Inactive") }
      ]}
    />
  );
}
